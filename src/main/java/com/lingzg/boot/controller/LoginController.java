...
import com.lingzg.boot.util.Captcha;
import com.lingzg.boot.util.GifCaptcha;
...
public class LoginController extends  BaseController{
    ...
    public String login(HttpServletRequest request,String username,String password,String vcode) {
    		...
    		try {  
	            ...
	            session.setAttribute("message", "未知账户");  
	        }catch(IncorrectCredentialsException ice){  
	            logger.info("对用户[" + username + "]进行登录验证..验证未通过,错误的凭证");  
	            session.setAttribute("message", "密码不正确");  
	        }catch(LockedAccountException lae){  
	            logger.info("对用户[" + username + "]进行登录验证..验证未通过,账户已锁定");  
	            session.setAttribute("message", "账户已锁定");
	        }catch(ExcessiveAttemptsException eae){  
	            logger.info("对用户[" + username + "]进行登录验证..验证未通过,错误次数过多");  
	            session.setAttribute("message", "用户名或密码错误次数过多");
